# ⚔️ Combat System & Engine Architect

"상황에 맞는 문제 정의와 본질적인 해결 방법을 고민하는 CPP 프로그래머 황효준입니다."

C++ 기반 로직 설계와 엔진 구조 이해를 바탕으로, 플레이어가 '손맛'을 느끼는 전투 시스템을 구현합니다.

## 🛠 Tech Stack
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![Unreal Engine](https://img.shields.io/badge/UnrealEngine-0E1128?style=flat&logo=unrealengine&logoColor=white)
![DirectX](https://img.shields.io/badge/DirectX-3D3D3D?style=flat&logo=microsoft&logoColor=white)
![Houdini](https://img.shields.io/badge/Houdini-FF4100?style=flat&logo=sidefx&logoColor=white)

## 📌 Featured Projects | 한 걸음씩 앞으로!
| 프로젝트 제목 | 핵심 기술 | 개발 기간 |
| :--- | :--- | :--- |
| K-Zombie FPS | GAS, Animation, AI | 26.04 ~ (진행중) |
| Context-Aware Animation | 논문 기반 상황 인식 시스템 | 26.06 ~ (예정) |
| DirectX Engine Demo | Graphics Pipeline | 26.02 ~ ( 연구중) |
| Virtual Idol Pipeline | 버츄얼 휴먼 사업화, 모션캡처 | 26.05 ~ (사업 진행중) |

## 🚩 Done Project Portfolio
| 프로젝트 제목 | 핵심 기술 | 개발 기간 |
| :--- | :--- | :--- |
| [XR 전시 : KickClay XR](https://youtu.be/fMi-oCwVEYk) | Unreal C++, XR Spouter, Hokoyo | 25.06 ~ 25.10 |
| [Steam : Beat & Beasties](https://store.steampowered.com/app/4267610/Beat__Beasties/) | 아트 총괄 및 플레이어블 개발 참여 | 25.09 ~ 26.03 |
| [Steam : QPopGirl](https://store.steampowered.com/app/4157070/QPopGirl/) | 아트 및 폴리싱 참여 | 24.12 ~ 24.05 |
| [RAG + QDrant + Streamlit](https://youtube.com/shorts/fpU0kqvtkbo) | RAG 기반 축구인 재활 프로토타입 | 25.11 ~ 25.12 |
| [자세 추정 점수화](https://youtu.be/2spXeHQ4doA) | 실시간 자세 추정/분석 프로토타입 | 25.07 ~ 25.10 |
| [VFX Portfolio](https://www.artstation.com/piusai) | 전투 집중한 UE4/UE5/Houdini VFX 작업물 | 24.04 ~ 25.03 |

## 🚀 Ongoing Research | 현재 R&D 진행중
- **Gameplay Ability System (GAS)**: 실무형 Ability 아키텍처 고도화
- **전통적인 Unreal FPS 전투 시스템**: 총기 로직, 타격감 구현 및 전투 로직 고도화 (Unreal Engine 5)
- **NC AI 3D Generation Integration**: 언리얼 엔진 모듈 연동 연구 (26 상반기~)

### 💡Research & Interest | 관심 연구 분야
*게임플레이 경험을 극대화하기 위한 기술적 난제들을 탐구합니다.*

* **Combat Feedback & Feel:** 공격-피격 간의 입력 지연(Input Latency) 최소화 및 콤보 시스템의 쾌적한 피드백 메커니즘
* **Animation System Integration:**  컨텍스트 인식(Context-Aware) 애니메이션을 활용한 동적 전투 상황 대응
* **Gameplay Architecture:** 대규모 환경에서의 GAS(Gameplay Ability System) 확장성 설계 및 컴포넌트 간 결합도 최적화
* **Performance Optimization:** CPU-GPU 병목 구간 제어와 데이터 중심 설계(Data-Oriented Design)를 통한 프레임 안정화

## 🎓 Education & Awards
- 2025 한국 게임학회 추계 학술대회 논문 우수상(25.10) : [언리얼 엔진5 기반 하이브리드 프로젝션 XR 환경 성능 최적화](https://github.com/piusAI/piusAI/blob/main/Performance%20Optimization%20of%20a%20Hybrid%20XR%20Environment%20Based%20on%20Unreal%20Engine%205.pdf)
- **홍익대학교 게임학부 석사 과정** : 25.09 ~ 27.08

## 📧 Contact
- **Email:** hwangpiusjoon@gmail.com
- **Blog:** [기술 블로그 링크]
